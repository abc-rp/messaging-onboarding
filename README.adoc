...
Register a tenant using Hono’s Device Registry’s management HTTP API (unless you are the admin of the IOT instance skip this step):
...
IMPORTANT: Rather then setting the device ID to the `BDNS` id we could let hono generate a UUID and include the `BDNS` id as metadata.
