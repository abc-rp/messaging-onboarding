...
This documentation is designed as a walk through to onboard user to the ABC-RP messaging stack.
It will demonstrate an exempla onboarding process as well as linking to further reading regarding the APIs and standards employed.

Efforts have been made to link to rather then duplicate other documentation.
...
The most straight forward scenario is a device connecting to one of Hono’s protocol adapters directly via IP based network infrastructure.
...
This gateway, for example, may handel communications for the `GB-LON-BLD2_TSTAT-1` sensor mentioned above.
...
For a `TPS` temperature sensor device we may use Brick data point names from the link:https://brickschema.org/ontology/1.1/classes/Air_Temperature_Sensor[Air Temperature Sensor] sub class of:
...
TIP: Arup provide a useful helper library link:https://github.com/arupiot/pyudmi[pyudmi] for working with UDMI objects in python.
